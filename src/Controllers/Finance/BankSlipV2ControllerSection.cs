...
{
    /// <summary>
    /// Authenticated client for the parallel bank slip v2 API.
    /// </summary>
    public sealed class BankSlipV2ControllerSection : AuthenticatedControllerSection
    {
        private const string Prefix = "/v2/finance/bankslip";
        private readonly JsonSerializerOptions _json;

        public BankSlipV2ControllerSection(IAuthenticatedControllerBase controllerBase)
            : base(controllerBase)
        {
            _json = controllerBase.Json;
        }

        public async Task<BankSlipV2View?> RequestAsync(
            ...
            CancellationToken cancellationToken)
        {
            ...
            try
            {
                return await response.Content
                    .ReadFromJsonAsync<BankSlipV2View>(_json, cancellationToken)
        ...
        public Task<BankSlipV2View?> GetAsync(
            Guid bankSlipId,
            CancellationToken cancellationToken)
        {
            var uri = $"{Prefix}?bankSlipId={Uri.EscapeDataString(bankSlipId.ToString("D"))}";
            return Request<BankSlipV2View>(
                new HttpRequestMessage(HttpMethod.Get, uri),
                cancellationToken);
        }

        public Task<BankSlipV2PayerReadiness?> GetPayerReadinessAsync(
            Guid contextId,
            string? provider,
            CancellationToken cancellationToken)
        {
            ...
            return Request<BankSlipV2PayerReadiness>(
                new HttpRequestMessage(HttpMethod.Get, uri),
                cancellationToken);
        }

        public Task<BankSlipV2SearchResult?> SearchAsync(
            BankSlipV2SearchParameters parameters,
            CancellationToken cancellationToken)
        {
            var uri = $"{Prefix}/search?{parameters.ToQueryString()}";
            return Request<BankSlipV2SearchResult>(
                new HttpRequestMessage(HttpMethod.Get, uri),
                cancellationToken);
        }

        public Task<BankSlipV2Statistics?> GetStatisticsAsync(
            BankSlipV2StatisticsParameters parameters,
            CancellationToken cancellationToken)
        {
            var uri = $"{Prefix}/statistics?{parameters.ToQueryString()}";
            return Request<BankSlipV2Statistics>(
                new HttpRequestMessage(HttpMethod.Get, uri),
                cancellationToken);
        }
        ...
        public Task<BankSlipV2SettingsView?> GetSettingsAsync(
            CancellationToken cancellationToken)
            => Request<BankSlipV2SettingsView>(
                new HttpRequestMessage(HttpMethod.Get, $"{Prefix}/settings"),
                cancellationToken);

        public Task<BankSlipV2SettingsView?> UpdateSettingsAsync(
            ...
            CancellationToken cancellationToken)
        {
            ...
            return Request<BankSlipV2SettingsView>(message, cancellationToken);
        }
        ...
        public Task<BankSlipV2View?> CancelAsync(
            Guid bankSlipId,
            CancellationToken cancellationToken)
        {
        ...
        public Task<BankSlipV2View?> RetryAsync(
            Guid bankSlipId,
            CancellationToken cancellationToken)
        {
